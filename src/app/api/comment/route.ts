...
export async function POST(req:Request){
   try {
      ...
      if(existBlog.authorId===session.user.id && !body.parentId){
         return NextResponse.json({ message: "Blog authors cannot comment on their own blog directly." }, { status: 403 });
      }
      if(body.parentId){
        ...
        return NextResponse.json({ message: "Nested replies are limited to 1 level" }, { status: 400 });
       }
      }
