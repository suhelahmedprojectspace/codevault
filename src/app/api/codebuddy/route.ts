...
//   matchDetails: string[];
// }
const MatchingPrefenceSchema = z
  .object({
  ...
  try {
    ...
    if (!validation.success) {
      return NextResponse.json(
        { message: 'Invalid preferences', errors: validation.error.errors },
        { status: 400 },
      );
    }
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ matches: sortedMatches.slice(0, 10) }, { status: 200 });
  } catch (error) {
