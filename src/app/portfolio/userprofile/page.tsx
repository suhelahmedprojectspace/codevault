...
export default function PortfolioPage() {
  ...
  useEffect(() => {
    const fetchPortfolio = async () => {
      setProgress(10);
      const response = await axios.get(`/porfolio`, {
      ...
      setProgress(90);
      setTimeout(() => {
        setOwner(response.data.portfolio.userid);
        ...
        {session?.user?.id === owner && (
          <div className="fixed z-20 top-10 right-4">
            <Button
              type="button"
              variant="outline"
              className="gap-2 text-zinc-800 font-semibold"
              onClick={() => router.push('/dashboard/portfolio/editportfolio')}
            >
    ...
    >
      ...
      <section id="contact" className="py-20 px-6 max-w-5xl mx-auto">
        ...
        >
          {SOCIAL_MEDIA_PLATFORMS.map((platform) => {
            ...
            const platformName = platform.label.replace('X (Twitter)', 'Twitter');
      ...
      <footer className="bg-zinc-900/90 border-t border-white/10 backdrop-blur-sm py-12 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          {/* <EditCard open={showeditbutton} onClose={()=>setShoweditButton(true)} data={'dfafdfdsfdf'} type="dfdflafak"/> */}
          <motion.div
