...
export function calculateCompatibility(userA: User, userB: User) {
  ...
  if (prefsA.experienceLevel && prefsB.experienceLevel) {
    ...
    const diff = Math.abs(
      levels.indexOf(prefsA.experienceLevel) - levels.indexOf(prefsB.experienceLevel),
    );
  ...
  if (prefsA.projectGoals && prefsB.projectGoals) {
    const commonGoals = prefsA.projectGoals.filter((goal) => prefsB.projectGoals!.includes(goal));

    const matchRatio =
      commonGoals.length / Math.max(prefsA.projectGoals.length, prefsB.projectGoals.length);
  ...
  if (prefsA.tools && prefsB.tools) {
    const commonTools = prefsA.tools.filter((tools) => prefsB.tools!.includes(tools));

    const matchRatio = commonTools.length / Math.max(prefsA.tools.length, prefsB.tools.length);
